from io import BytesIO
from itertools import cycle
import operator
import sys
 
'''
   Put this script in the same directory as data.pack and run `python3 xor.py`
   
   Does not decrypt all sub files, some of the contents of data.pack are further encrypted.
'''
def xore(data, key):
    return bytes(a ^ b for a, b in zip(data, cycle(key)))
key = b''
 
stream = BytesIO(open("data.pack", "rb").read())
with open("output_result", "wb") as f:
    while True:
        de = xore(stream.read(len(key)), key)
        f.write(de)
        if de == b'':
            print("DONE")
            break
